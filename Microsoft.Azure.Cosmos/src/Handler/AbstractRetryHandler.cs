            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    try
                    {
                        ResponseMessage cosmosResponseMessage = await base.SendAsync(request, cancellationToken).ConfigureAwait(false);
                        if (cosmosResponseMessage.IsSuccessStatusCode)
                        {
                            return cosmosResponseMessage;
                        }

                        result = await retryPolicyInstance.ShouldRetryAsync(cosmosResponseMessage, cancellationToken).ConfigureAwait(false);
                    ...
                    {
                        result = await retryPolicyInstance.ShouldRetryAsync(httpRequestException, cancellationToken).ConfigureAwait(false);
                        if (!result.ShouldRetry)
                        {
                    ...
                    {
                        await Task.Delay(result.BackoffTime, cancellationToken).ConfigureAwait(false);
                    }
                }
            }
